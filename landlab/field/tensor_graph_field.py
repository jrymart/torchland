...
# no change needed to work with tensors
...
# changed to work with tensors
# tensor.size is not equivlent to np.size
# equiv value for tensor calculated and then used throughout function
def shape_for_storage(array, field_size=None):
    ...
    ValueError: unable to reshape array to field size
    """
    array_size = np.product(array.size())
    if field_size is None:
        field_size = array_size

    if array_size  % field_size != 0:
        raise ValueError(
    ...
    if field_size == array_size              :
        shape = (array_size,)
    else:
        shape = (field_size, array_size // field_size)
...
class FieldDataset(dict):
    ...
    def set_value(self, name, value_array, attrs=None):
        ...
        self._ds.update({name: value_array})
